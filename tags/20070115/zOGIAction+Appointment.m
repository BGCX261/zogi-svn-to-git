...
-(NSDictionary *)_renderAppointment:(NSDictionary *)eoAppointment
  ...
  /* Commented out since it currently produces an exception
      Invalid argument: Nil object to be added in array
     The conflict code has been extremely problematic
...
-(NSArray *)_getUnrenderedDatesForKeys:(id)_arg {
  [self logWithFormat:@"_getUnrenderedDatesForKeys;_arg=%@", _arg];
...
-(id)_searchForAppointments:(NSDictionary *)_query 
  ...
  [self logWithFormat:@"_searchForAppointments"];
  ...
  [startDate setTimeZone:[self _getTimeZone]];
  [self logWithFormat:@"startDate initialized"];
  ...
  [endDate setTimeZone:[self _getTimeZone]];
  [self logWithFormat:@"endDate initialized"];
  ...
  */
  ...
  [self logWithFormat:@"%d dates found from search", [gids count]];
...
-(id)_writeAppointment:(NSDictionary *)_appointment
  ...
  [[self getCTX] commit];
  [self logWithFormat:@"_writeAppointment(...): committed"];
...
-(id)_translateAppointment:(NSDictionary *)_appointment 
  ...
  NSArray               *keys;
  NSString              *key;
  id                    objectId, value, participants;
  int                   count;
  ...
  for (count = 0; count < [keys count]; count++) {
    ...
    [self logWithFormat:@"_translateAppointment(); translating key %@", key];
    ...
    if ([key isEqualToString:@"end"]) {
      ...
      [appointment setObject:[self _makeCalendarDate:value] forKey:@"cycleEnd"];
      ...
      [appointment setObject:value forKey:@"parentDateId"];
